...
- **Virtual scroll** (`miscVirtualScroll.ts`): DOM body height + smooth wheel snapping. Drives scroll position only — does NOT call `ChangeSection`. Initialized from `App.vue` via `InitializeVirtualScroll(count, vh)`. Owns the **scroll gate**: one ±1 section step per gesture; all scroll input is locked from the moment a stroke fires (and on every `LockTransition` in `sectionsCore`) until `sectionsCore` signals release via `releaseVirtualScrollAfterSectionTransition(currentSection)`, which realigns the scroll layer to the authoritative section. Programmatic jumps must call `syncVirtualScrollToSection(index)` (done in `ChangeToSectionID`).
